...
### 📚 보안 교육 모듈 (실전 보안 콘텐츠)
- **고정 9문항** — 3개 카테고리 × 3문제, 한 판에 9문항이 모두 출제
- NPC 대화 종료 시 해당 카테고리 **3문제 연속 출제** (문제·선택지 순서만 셔플)
...
| 카테고리 | 문항 | 다룬 주제 (출처) |
|---|---|---|
| 🔬 외부 매체 / 자료 관리 (`research_usb`) | 3 | 미상 USB 습득 대응, 공공 와이파이 VPN 접속, 개인 클라우드 백업 금지 |
| 💻 네트워크 / 인증 (`server_log`) | 3 | 임원 사칭 피싱(스피어피싱), **크리덴셜 스터핑**(비밀번호 재사용), 2차 인증(OTP) |
| 🔑 물리 보안 (`cardkey_log`) | 3 | 출력물 파쇄 폐기, 자리 비울 때 화면 잠금(세션 차단), **테일게이팅** 대응 |
